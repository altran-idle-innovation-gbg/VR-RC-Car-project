...
class Car(object):
    ...
    def set_driving_direction(self, drivingDirection):
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == K_w:  # key <W> Move forward
                    theCar.set_driving_direction('forward')
                    print(theCar.get_driving_direction())
                    # driveForward()
                if event.key == K_s:  # key <S> Move backward
                    theCar.set_driving_direction('backward')
                    print(theCar.get_driving_direction())
                    # driveBackward()
                if event.key == K_a:  # key <A> Move left
                    theCar.set_driving_direction('left')
                    print(theCar.get_driving_direction())
                    # driveLeftForward()
                if event.key == K_d:  # key <D> Move right
                    theCar.set_driving_direction('right')
                    print(theCar.get_driving_direction())
                    # driveRightForward()
                ...
                if event.key == K_c:
                    ...
                    print('Camera Direction DC = ', theCar.get_camera_direction())
                if event.key == K_v:
                ...
                theCar.set_driving_direction('stop')
                print(theCar.get_driving_direction())
                # stopAll()
            drivingDirectionList[theCar.get_driving_direction()]()
            pwm.ChangeDutyCycle(theCar.get_camera_direction())
            time.sleep(0.05)
